...
image::./architecture/thumbs/images/concepts/outbox-pattern.png[]

We can face problems when we're trying to publish our events, and with this keep the system consistent with wrong status, to solve this, we need combine SAGA with Outbox Pattern on schedule, to keep SAGA state under the outbox table with Outbox Pattern, we don't publish event directly on broker, but instead of, we keep our events in the local database/table called **Outbox Table**, this table belongs to the same database that we're using to persist the operations/status based on ACID transactions with additional event inserted on _Outbox Table_ using same ACID transaction, after that we can read data/event from _Outbox table_ and finally publish the event, using pulling data or CDC, CDC will listen each transaction log under the _Outbox Table_. In both cases we should do the implementation to be sure that you don't miss the events in the Outbox table. Using this approaches we'll keep track of SAGA and Order Status for each operation using _outbox table_ ensuring idempotency to prevent data corruption, with optimistic op and constraints on table
